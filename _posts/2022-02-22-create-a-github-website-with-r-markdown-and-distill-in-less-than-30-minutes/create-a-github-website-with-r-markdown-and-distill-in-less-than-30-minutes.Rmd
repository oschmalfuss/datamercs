...
Following a minimalist approach to a not sucking, uncluttered internet, I was one of those guys with HTML only pages or simple Google sites. But over time I found that approach neither as efficient nor fun any longer and thus looked for other alternatives to get some of my older posts back online again.  
In comes RStudio that I started heavily using for automating reports and as an escape route from MS Excel. Working with R Markdown anyway I started looking into blogdown and then came across Distill in one of the tutorials I watched.  
...
The advantage of using Distill is that we have the logic of utilizing a blog built in and that we do not have the need to maintain links all over the place manually.  It's not for people in need of a complex set-up though, with lots of control over layout or other advanced web features.  
One step of complexity and customizability more towards that, would be the use of blogdown and Hugo, possibly in tandem with Netlify, which is a typical set-up you will see in the community.  
